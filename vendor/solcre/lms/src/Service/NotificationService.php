...
class NotificationService extends BaseService
{
    ...
    //APN Configuration
    const APN_PEM_FILE = '/pushcert.pem';
    const APN_PEM_PASSPHRASE = 'com.solcre.lms';
    ...
    public function __construct(
        EntityManager $entityManager, UserService $userService, DeviceService $deviceService, $config
    ) {
    ...
    public function add($data, $strategies = null, $flush = true)
    {
        ...
        $notification = parent::prepareEntity($data);
    ...
    public function fetchAll($params = null, $orderBy = null)
    {
        ...
        if ($user instanceof UserEntity)
        {
            ...
            {
                if ($userNotification->getType() === $this->config['lms']['PUSH_NOTIFICATIONS']['TYPES']['NOTIFICATION_TYPE_GENERAL'])
                {
    ...
    private function addRecipients(AndroidMessage &$message, array $devices)
    {
    ...
    public function sendIosNotification(int $notificationId, array $devices)
    {
        $notification = $this->fetch($notificationId);
    ...
    private function checkIosDeviceToken($deviceToken)
    {
